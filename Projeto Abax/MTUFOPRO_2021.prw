...
If Empty(cIdAbx)
   ...
   lthread := .T. 
	...
	Do While SM0->(!Eof())	
	 	...
	 	If AllTrim(SM0->M0_CODIGO) $ GetNewPar("MV_ZEMPABX", "01") // Parâmetro empresas Urbano
	...
	Do While ZNFTEMP->(!Eof())
		...
		If !lImport		                                 
         ...
         Endif
		    ...
		    Aadd(_aCabSF1,{"F1_MODNF"    ,ZNFTEMP->ZNF_MODNF			   			,Nil}) 	//DESBH       
         ...
         Endif
	      ...
	      If ZNFTEMP->ZNF_PBRUTO > 0
   	    ...
   	    Aadd(_aCabSF1,{"F1_ESTPRES"  ,ZNFTEMP->ZNF_ESTPRE				   		,Nil})  //Estado do Municipio 	   			              
			...
			If ZNFTEMP->ZNF_ESPEC = 'NFSE' .or. ZNFTEMP->ZNF_ESPEC = 'NFPS'// IncluÃ­do tratamento de Fornecedor de ISS e Loja ISS
	   ...
	   dbSelectArea('SC7')  
		...
		If ZNFTEMP->ZNF_BSIPI > 0	
			...
			Aadd(_aLinha,{"D1_IPI" 		 ,ZNFTEMP->ZNF_PIPI	 							,Nil})	//Valor do IPI - ALIQUOTA 243			
				...
				Begin transaction 
					...
					Else                        
					   //InclusÃ£o de Nota Fiscal
						MSExecAuto({|x,y,z| MATA103(x,y,z)              },_aCabSF1,_aItensSD1,3   ,         ,aCodRet)    						
					Endif
