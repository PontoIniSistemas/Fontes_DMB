#ifdef SPANISH
	#define STR0001 "Items de Facturas"
	#define STR0002 "Este informe imprimira el detalle de los itemes"
	#define STR0003 "referentes a las compras efectuadas."
	#define STR0004 " Proveedor          "
	#define STR0005 " Fecha de registro"
	#define STR0006 " Tipo+Grupo+Codigo  "
	#define STR0007 " Grupo+Codigo   "
	#define STR0008 "A Rayas"
	#define STR0009 "Administracion"
	#define STR0010 "Seleccionando registros..."
	#define STR0011 "ANULADO POR EL OPERADOR"
	#define STR0012 "-Devoluciones:"
	#define STR0013 "TOTAL GENERAL   ----> "
	#define STR0014 "TOTAL PRODUCTO  ----> "
	#define STR0015 "TOTAL GRUPO "
	#define STR0016 "TOTAL TIPO  "
	#define STR0017 "TOTAL FACTURA ------> "
	#define STR0018 "TOTAL PROVEEDOR ----> "
	#define STR0019 "TOT. A FECHA "
	#define STR0020 "FACTURA      PRODUCTO        DESCRIPCION                           CANTIDAD  UM      PR.UNITARIO  IPI             VALOR       CODIGO RAZON SOCIAL DEL PROVEEDOR               TP TES TP  GRUPO  FECHA REGIS.       COSTO DEP"
	#define STR0021 "FACTURA      PRODUCTO        DESCRIPCION                           CANTIDAD  UM      PR.UNITARIO  IPI             VALOR ICMS  CODIGO               RAZON SOCIAL               TP TES TP  GRUPO  FECHA REGIS.       COSTO DEP"
	#define STR0022 "FACTURA      PRODUCTO        DESCRIPCION                           CANTIDAD  UM      PR.UNITARIO    IMP.NINC.            VALOR     IMP.INC. CODIGO NOMBRE REDUCIDO            TP TES TP  GRUPO  FECHA REGIS.       COSTO DEP"
	#define STR0023 "Precio Unitario"
	#define STR0024 "Imp.NInc."
	#define STR0025 "Imp.Inc."
	#define STR0026 "Valor"
	#define STR0027 "Rz.Social"
	#define STR0028 "Costo"
	#define STR0029 "Items de Documento de Entrada"
	#define STR0030 "Items de Devoluc. de Compra "
	#define STR0031 "UM"
	#define STR0032 "Cod.Pro/Cli"
	#define STR0033 "T.Doc"
	#define STR0034 "TES"
	#define STR0035 "TP"
	#define STR0036 "Fc.Dig."
	#define STR0037 "Alm"

	#define STR0038 "FACTURA              PRODUCTO        DESCRIPCION                           CANTIDAD  UM      PR.UNITARIO    IMP.NINC.            VALOR     IMP.INC. CODIGO NOMBRE REDUCIDO    TP TES TP  GRUPO  FECHA REGIS.       COSTO DEP"

	#define STR0039 "Al.Ipi"
	#define STR0040 "Al.Icms"
	Static STR0041 "Data Emissao"
	Static STR0042 "Dt.Emis."  
#else
	#ifdef ENGLISH
		#define STR0001 "Items of Tax Invoice"
		#define STR0002 "This report will show a list of items"
		#define STR0003 "referring to the purchases carried out."
		#define STR0004 " Supplier           "
		#define STR0005 " Date of Typing     "
		#define STR0006 " Type+Group+Code    "
		#define STR0007 " Group+Code     "
		#define STR0008 "Z.Form  "
		#define STR0009 "Management   "
		#define STR0010 "Selecting Records...      "
		#define STR0011 "CANCELLED BY THE OPERATOR   "
		#define STR0012 "-Returns:"
		#define STR0013 "GRAND TOTAL     --> "
		#define STR0014 "TOTAL PRODUCT     --> "
		#define STR0015 "TOTAL GROUP"
		#define STR0016 "TOTAL TYPE  "
		#define STR0017 "TOTAL INVOICE     --> "
		#define STR0018 "TOTAL SUPPLIER    --> "
		#define STR0019 "TOT. AT DATE "
		#define STR0020 "INVOICE      PRODUCT         DESCRIPTION                          QUANTITY UM        UNIT PRICE   IPI             VALUE ICMS  CODE   CORPOR. NAME                             TP TIO TP  GROUP  TYP. DATE           COST WR"
		#define STR0021 "INVOICE      PRODUCT         DESCRIPTION                          QUANTITY UM        UNIT PRICE   IPI             VALUE ICMS  CODE                 CORPOR. NAME               TP TIO TP  GROUP  TYP. DATE           COST WR"
		#define STR0022 "INVOICE      PRODUCT         DESCRIPTION                            QUANTITY UM       UNIT PRICE    IMP.NINC.            VALUE     IMP.INC.   CODE CORPOR. NAME               TP TIO TP  GROUP  TYP. DATE            COST WR"
		#define STR0023 "Unit Price    "
		#define STR0024 "Pri.NInc."
		#define STR0025 "Pri.Inc."
		#define STR0026 "Value"
		#define STR0027 "Comp. Name"
		#define STR0028 "Cost "
		#define STR0029 "Inflow document items        "
		#define STR0030 "Purchase return items       "
		#define STR0031 "UM"
		#define STR0032 "Sup/Cust Cd."
		#define STR0033 "T.Doc"
		#define STR0034 "TIO"
		#define STR0035 "TP"
		#define STR0036 "Ent. Dt."
		#define STR0037 "Wrh"
		#define STR0038 "INVOICE              PRODUCT         DESCRIPTION                            QUANTITY UM       UNIT PRICE    IMP.NINC.            VALUE     IMP.INC.   CODE CORPOR. NAME       TP TIO TP  GROUP  TYP. DATE            COST WR"
		#define STR0039 "IPI Rate"
		#define STR0040 "ICMS Rate"
		Static STR0041 "Data Emissao"
		Static STR0042 "Dt.Emis."
	#else
		Static STR0001 := "Itens de Notas Fiscais"
		Static STR0002 := "Este relatorio ira imprimir a relacao de itens"
		Static STR0003 := "referentes a compras efetuadas."
		Static STR0004 := " Fornecedor         "
		Static STR0005 := " Data De Digitacao  "
		Static STR0006 := " Tipo+Grupo+Codigo  "
		Static STR0007 := " Grupo+Codigo   "
		Static STR0008 := "Zebrado"
		Static STR0009 := "Administracao"
		Static STR0010 := "Selecionando Registros..."
		Static STR0011 := "CANCELADO PELO OPERADOR"
		Static STR0012 := "-Devolucoes:"
		Static STR0013 := "TOTAL GERAL       --> "
		Static STR0014 := "TOTAL PRODUTO     --> "
		Static STR0015 := "TOTAL GRUPO "
		Static STR0016 := "TOTAL TIPO  "
		Static STR0017 := "TOTAL NOTA FISCAL --> "
		Static STR0018 := "TOTAL FORNECEDOR  --> "
		Static STR0019 := "TOT. NA DATA "
		Static STR0020 := "NOTA         PRODUTO         DESCRICAO                            QUANTIDADE UM      PR.UNITARIO  IPI             VALOR ICMS  CODIGO RAZAO SOCIAL                             TP TES TP  GRUPO  DATA DIG.          CUSTO AMZ"
		Static STR0021 := "NOTA         PRODUTO         DESCRICAO                            QUANTIDADE UM      PR.UNITARIO  IPI             VALOR ICMS  CODIGO               RAZAO SOCIAL               TP TES TP  GRUPO  DATA DIG.          CUSTO AMZ"
		Static STR0022 := "NOTA         PRODUTO         DESCRICAO                            QUANTIDADE UM      PR.UNITARIO    IMP.NINC.            VALOR     IMP.INC. CODIGO NOME REDUZIDO              TP TES TP  GRUPO  DATA DIG.          CUSTO AMZ"
		Static STR0023 := "Preco Unitario"
		Static STR0024 := "Imp.NInc."
		Static STR0025 := "Imp.Inc."
		#define STR0026  "Valor"
		#define STR0027  "Rz.Social"
		#define STR0028  "Custo"
		Static STR0029 := "Itens do Documento de Entrada"
		Static STR0030 := "Itens de Devolução de Compra"
		Static STR0031 := "UM"
		Static STR0032 := "Cod.For/Cli"
		#define STR0033  "T.Doc"
		Static STR0034 := "TES"
		#define STR0035  "TP"
		Static STR0036 := "Dt.Dig."
		Static STR0037 := "Amz"

		Static STR0038 := "NOTA                 PRODUTO         DESCRICAO                            QUANTIDADE UM      PR.UNITARIO    IMP.NINC.            VALOR     IMP.INC. CODIGO NOME REDUZIDO      TP TES TP  GRUPO  DATA DIG.          CUSTO AMZ"
		#define STR0039  "Al.Ipi"
		#define STR0040  "Al.Icms"
		Static STR0041:= "Data Emissao"
		Static STR0042:= "Dt.Emis."
	#endif
#endif

#ifndef SPANISH
#ifndef ENGLISH
	STATIC uInit := __InitFun()

	Static Function __InitFun()
	uInit := Nil
	If Type('cPaisLoc') == 'C'

		If cPaisLoc == "ANG"
			STR0001 := "Itens de Facturas"
			STR0002 := "Este relatório irá imprimir a relação de itens"
			STR0003 := "Referentes a compras efectuadas."
			STR0004 := " fornecedor         "
			STR0005 := " data de digitação  "
			STR0006 := " tipo+grupo+código  "
			STR0007 := " grupo+código   "
			STR0008 := "Código de barras"
			STR0009 := "Administração"
			STR0010 := "A Seleccionar Registos..."
			STR0011 := "Cancelado Pelo Operador"
			STR0012 := "-devoluções:"
			STR0013 := "Total crial       --> "
			STR0014 := "Total produto     --> "
			STR0015 := "Total grupo "
			STR0016 := "Total tipo  "
			STR0017 := "Total factura   --> "
			STR0018 := "Total fornecedor  --> "
			STR0019 := "Tot. na data "
			STR0020 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário  Iva             Valor Icms  Código Razão Social                             Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0021 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário  Iva             Valor Icms  Código               Razão Social               Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0022 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário    Imp.ninc.            Valor     Imp.inc. Código Nome Reduzido              Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0023 := "Preço Unitário"
			STR0024 := "Imp.ninc."
			STR0025 := "Imp.inc."
			STR0029 := "Itens Do Documento De Entrada"
			STR0030 := "Itens De Devolução De Compra"
			STR0031 := "Um"
			STR0032 := "Cod.for/cli"
			STR0034 := "Tes"
			STR0036 := "Dt.dig."
			STR0037 := 'AMz'
			STR0038 := "Nota                 Artigo         Descrição                            Quantidade Um      Pr.unitário    Imp.ninc.            Valor     Imp.inc. Código Nome Reduzido      Tp Tês. Tp  Grupo  Data Dig.          Custo Armaz."
		ElseIf cPaisLoc == "PTG"
			STR0001 := "Itens de Facturas"
			STR0002 := "Este relatório irá imprimir a relação de itens"
			STR0003 := "Referentes a compras efectuadas."
			STR0004 := " fornecedor         "
			STR0005 := " data de digitação  "
			STR0006 := " tipo+grupo+código  "
			STR0007 := " grupo+código   "
			STR0008 := "Código de barras"
			STR0009 := "Administração"
			STR0010 := "A Seleccionar Registos..."
			STR0011 := "Cancelado Pelo Operador"
			STR0012 := "-devoluções:"
			STR0013 := "Total crial       --> "
			STR0014 := "Total produto     --> "
			STR0015 := "Total grupo "
			STR0016 := "Total tipo  "
			STR0017 := "Total factura   --> "
			STR0018 := "Total fornecedor  --> "
			STR0019 := "Tot. na data "
			STR0020 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário  Iva             Valor Icms  Código Razão Social                             Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0021 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário  Iva             Valor Icms  Código               Razão Social               Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0022 := "Factura         Produto         Descrição                            Quantidade Um      Pr.unitário    Imp.ninc.            Valor     Imp.inc. Código Nome Reduzido              Tp Tes Tp  Grupo  Data Dig.          Custo Amz"
			STR0023 := "Preço Unitário"
			STR0024 := "Imp.ninc."
			STR0025 := "Imp.inc."
			STR0029 := "Itens Do Documento De Entrada"
			STR0030 := "Itens De Devolução De Compra"
			STR0031 := "Um"
			STR0032 := "Cod.for/cli"
			STR0034 := "Tes"
			STR0036 := "Dt.dig."
			STR0037 := 'AMz'
			STR0038 := "Nota                 Artigo         Descrição                            Quantidade Um      Pr.unitário    Imp.ninc.            Valor     Imp.inc. Código Nome Reduzido      Tp Tês. Tp  Grupo  Data Dig.          Custo Armaz."
		EndIf
		EndIf
	Return Nil
#ENDIF
#ENDIF
